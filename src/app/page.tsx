    ...
    <div className="min-h-screen bg-[#0a0a0f]">
        ...
        <section>
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <ul className="text-xs text-[#71717a] space-y-1 list-disc list-inside">
              <li><strong className="text-[#f4f4f6]">6 Occupied / 3 Reserved / 14 Available</strong> — door utilization is task-derived at the 2026-09-22 ~10:09 PDT snapshot: 6 doors with an in-progress open task (DOCK50, DOCK51, DOCK53, DOCK54, DOCK57, DOCK62), 3 doors holding only not-started NEW tasks (DOCK55, DOCK56, DOCK59 — Reserved rather than Occupied), 14 doors with no open load/receive task. 9 doors carry an active assignment (39.1% of the bay). <strong className="text-[#f4f4f6]">Cross-check / correction candidate:</strong> the Location API&apos;s own <code>dockStatus</code> field reports 14 Bay-4 doors OCCUPIED / 2 RESERVED / 7 AVAILABLE. It is driven by trailer check-in state rather than by open dock work and <strong className="text-[#f4f4f6]">diverges materially</strong> from task-derived status (6 vs 14). Task-derived status is retained as the authoritative basis for this dashboard, consistent with the door-duration metric.</li>
              <li>Open tasks: <strong className="text-[#7c3aed]">5 outbound (LOAD)</strong> / <strong className="text-[#22c55e]">7 inbound (RECEIVE)</strong> = 12 at ~10:09 PDT. 41.7% outbound / 58.3% inbound. The open population was read exhaustively — a full facility-wide sweep ({facilityOpenLoad} open load + {facilityOpenReceive} open receive tasks, statuses NEW/IN_PROGRESS/EXCEPTION) filtered on each row&apos;s <code>dockId</code> to the 23 Bay-4 doors.</li>
              <li><strong className="text-[#7c3aed]">Customer mix:</strong> 9 of the 12 open Bay-4 tasks are GURUNANDA, LLC (ORG-655875) — 75.0%; 3 are KARAKA, LLC (ORG-585450) — DOCK55 TASK-5373122, DOCK55 TASK-5368665, DOCK56 TASK-5369120. Task status: 7 IN_PROGRESS + 5 NEW.</li>
              <li><strong className="text-[#ef4444]">SEVERE ANOMALY — DOCK50 (aging 335d 20h 47m):</strong> TASK-5090739 RECEIVE remains IN_PROGRESS with endTime already set (2025-10-22) — stuck/stale and still aging. Assigned to daira gonzalez. Investigate immediately.</li>
              <li><strong className="text-[#f59e0b]">ANOMALY — DOCK54 (aging 45d 17h 39m and 11d 0h 36m):</strong> TASK-5338695 (LOAD, endTime 2026-08-10) and TASK-5365421 (LOAD, endTime 2026-09-11) both remain IN_PROGRESS — stale — on a door that retains two open load tasks for ARNULFO MUNGUIA.</li>
              <li><strong className="text-[#22c55e]">Live churn during this pull:</strong> none observed within the sweep — all figures on this page come from one tight, self-consistent sweep at 2026-09-22T17:08:53Z (2026-09-22 ~10:09 PDT).</li>
              <li><strong className="text-[#22c55e]">Schedule % REPORTED this refresh:</strong> today is <strong className="text-[#f4f4f6]">Tuesday 2026-09-22</strong> and the appointment windows for this facility-local day return live rows on both sides — <strong className="text-[#f4f4f6]">110 loads scheduled</strong> (24 loaded → <strong className="text-[#f4f4f6]">21.8%</strong>) and <strong className="text-[#f4f4f6]">31 receipts scheduled</strong> (0 received → <strong className="text-[#f4f4f6]">0.0%</strong>). Binding filters confirmed: <strong className="text-[#f4f4f6]">loads</strong> use <code>appointmentTimePeriod</code> — a BETWEEN that requires exactly two date-time elements, e.g. [&quot;2026-09-22T00:00:00&quot;,&quot;2026-09-22T23:59:59&quot;]; <strong className="text-[#f4f4f6]">receipts</strong> use <code>appointmentTimeFrom</code>/<code>appointmentTimeTo</code>. &quot;Loaded&quot; counts loads whose status shows loading complete — 17 SHIPPED + 7 LOADED = 24 (13 WINDOW_CHECKIN_DONE + 5 LOADING + 68 NEW still pending). &quot;Received&quot; counts receipts fully closed — 0 CLOSED + 0 FORCE_CLOSED = 0 (14 IMPORTED + 10 OPEN + 7 IN_PROGRESS still pending).</li>
              <li><strong className="text-[#22c55e]">All-time cumulative figures RECOMPUTED this refresh:</strong> the <strong className="text-[#f4f4f6]">{allTimeClosedTotal.toLocaleString()}</strong> closed Bay-4 transactions across <strong className="text-[#f4f4f6]">{allTimeDistinctAssignees}</strong> display names (top-10 list above) and the GURUNANDA → Arnulfo cumulative <strong className="text-[#f4f4f6]">{guruArnulfoAllTimeTotal}</strong> ({guruArnulfoAllTimeLoad} LOAD + {guruArnulfoAllTimeReceive} RECEIVE) come from a full row-level rescan performed in this refresh — 23 doors × 2 task types (2,827 LOAD + 1,005 RECEIVE). They are fresh, not carried forward.</li>
              ...
              <li><strong className="text-[#f59e0b]">TIMESTAMP BASIS — durations aged task startTime → snapshot instant:</strong> task timestamps are read on the same UTC frame as the snapshot instant (2026-09-22T17:08:53Z). Query window 2026-09-22T17:08:53Z → 2026-09-22T17:08:53Z.</li>
      ...
      <footer className="border-t border-[#1e1e2a] bg-[#0a0a0f] mt-2">
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between text-xs text-[#71717a]">
          ...
          <span>Last refreshed: {refreshDateLong} ~10:09 PDT</span>
        </div>
      </footer>
    </div>
  );
