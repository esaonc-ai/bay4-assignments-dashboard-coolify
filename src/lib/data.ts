 ...
 *
 * TASK DATA: Refreshed 2026-09-07 ~18:40 PDT (live WISE/WMS APIs)
 *   Sources:
 ...
 *
 * DELTA vs 2026-09-07 ~17:44 PDT snapshot: NO task opened, closed, or moved doors; NO schedule or
 * facility-context change (receipts created today still 9 - identical list; loads created today still 0;
 * 1 scheduled inbound RN-5009857 still OPEN -> 0 received; 0 scheduled outbound loads). Only task ages
 * advanced (~56m): TASK-5360206 4d1h->4d2h, TASK-5359541 4d7h->4d8h, TASK-5361270 3d1h->3d2h,
 * TASK-5360934 3d4h->3d5h, TASK-5338695 31d1h->31d2h, TASK-5090739 ~321d (321d 4h->321d 5h). All-time
 * Bay-4 closed rollup unchanged: 3,639 (2,718 LOAD + 921 RECEIVE) / 80 assignees; GURUNANDA -> Arnulfo
 * closed: 1,104 facility-wide (1,102 LOAD + 2 RECEIVE) / 901 Bay-4 (900 + 1). No closures since
 * 09-05 06:13:25Z (TASK-5361283, endTime local 09-04T23:13:25). Timestamps: BAM APIs return task
 * startTime/endTime/createdTime and receipt times as facility-local naive (America/Los_Angeles, UTC-7)
 * when item-time-zone=America/Los_Angeles — durations computed against the ~18:40 PDT snapshot instant,
 * same convention as the Sep 4/5/6/7 snapshots. Facility "today" windows use item-time-zone
 * America/Los_Angeles (local-day) semantics, matching prior refreshes.
 */
...
export const refreshStamp = "Sep 07 ~18:40 PDT";
...
// identical set to the 09-07 17:44 PDT snapshot: no task opened, closed, or moved doors in the
// window; ages advanced ~56m)
...
// FRESH rollup 2026-09-07 ~18:40 PDT — rescanned CLOSED/FORCE_CLOSED load + receive tasks
...
// 09-07 07:57/12:27/15:53/17:44 PDT): no Bay-4 closures in the window; newest closed task endTime still
// 2026-09-04T23:13:25 local = 09-05 06:13:25Z (TASK-5361283, RECEIVE, DOCK58) — re-verified at ~18:40 PDT
...
// 09-07 17:44 PDT snapshot; ages advanced ~56m)
...
// appointment entity 255079 / APPT-6037605 CHECKED_IN, inYard 07:36:02 PDT — OPEN, not yet received →
// 0/1; receive task TASK-5361525 NEW at DOCK152 (ERICK OLMEDO, created 07:29 PDT, outside Bay 4));
// 0 scheduled outbound loads (no load carries an appointmentTime in the 09-07 window) → outbound % n/a.
// (An OUTBOUND appointment entity exists — 256422, APPT-6037851, ORGAIN, LLC (ORG-655338), 13:00 PDT,
// CHECKED_IN — but its load record LOAD-5037118 has appointmentTime 09-02T10:00 (already SHIPPED), so per
// the load-search convention it does not create a denominator.) Endpoints verified live on 09-07 ~18:40 PDT.
...
// Today’s facility-wide context (2026-09-07, ~18:40 PDT)
...
//   — identical list to the 12:27 PDT snapshot (no receipts created 17:44→18:40 PDT)
...
//   LOAD-5038142 (NEW, created 2026-09-06T00:37:59 PDT)
...
// Open task records from fresh WISE data (Sep 07, 2026 ~18:40 PDT)
// 8 open tasks: 6 LOAD (outbound) + 2 RECEIVE (inbound) — identical set to the 09-07 17:44 PDT
// snapshot (no task opened, closed, or moved doors in the window; ages advanced ~56m)
export const assignments: TaskRecord[] = [
  ...
  },
  {
    ...
    dns: "LOAD IN_PROGRESS",
    customer: "GURUNANDA, LLC",
    pieces: "IN_PROGRESS (31d 2h) ⚠ STALE",
    assignee: "ARNULFO MUNGUIA",
    door: "DOCK54",
  },
  {
