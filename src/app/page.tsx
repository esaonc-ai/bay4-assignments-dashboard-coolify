    ...
    <div className="min-h-screen bg-[#0a0a0f]">
        ...
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-xs text-[#71717a] tracking-wide">
              DOCK50–DOCK72 &nbsp;|&nbsp; September 7, 2026 &nbsp;|&nbsp; Last refreshed: Sep 07 ~18:40 PDT
            </p>
          </div>
          {/* Facility badge */}
          ...
          <div className="mt-3 bg-[#141419] border border-[#1e1e2a] rounded-xl overflow-hidden">
            <div className="px-5 py-2.5 bg-[#0a0a0f] border-b border-[#1e1e2a]">
              <span className="text-xs font-semibold text-[#71717a] uppercase tracking-wider">
                All-Time Assignments (DOCK50–DOCK72) — recomputed 09/07 ~18:40 PDT
              </span>
            </div>
          ...
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <div className="bg-[#7c3aed10] border border-[#7c3aed33] rounded-lg px-4 py-3 mt-1">
              ...
              <span className="text-xs text-[#a1a1aa] block mt-0.5">
                ...
                (901 at Bay 4 doors) — fresh rollup, 2026-09-07 ~18:40 PDT. Below is the current snapshot.
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-[#a1a1aa] mt-1">
                  <strong>DOCK50:</strong> TASK-5360206 (LOAD, IN_PROGRESS, 4d 2h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK51:</strong> TASK-5359541 (LOAD, IN_PROGRESS, 4d 8h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK53:</strong> TASK-5361270 (LOAD, IN_PROGRESS, 3d 2h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK54:</strong> TASK-5360934 (LOAD, IN_PROGRESS, 3d 5h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK54:</strong> TASK-5338695 (LOAD, STALE, 31d 2h)
                </span>
        ...
        <section>
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <ul className="text-xs text-[#71717a] space-y-1 list-disc list-inside">
              <li><strong className="text-[#f4f4f6]">4 Occupied / 1 Reserved / 18 Available</strong> — door utilization sourced from open load/receive tasks: 4 doors with an active IN_PROGRESS task (DOCK50,51,53,54), 1 door with a NEW not-yet-started task (DOCK67), 18 doors with no open load/receive task. <strong className="text-[#a1a1aa]">Unchanged since the 09-07 17:44 PDT snapshot</strong> — no door opened, freed, or moved tasks in the window (~56m). (Note: the location API still reports a stale dockStatus=OCCUPIED for several task-free doors — DOCK52/55/59/61/65/66/68/69/70/71; task-derived status is authoritative, consistent with prior snapshots.)</li>
              <li>Open tasks: <strong className="text-[#7c3aed]">6 outbound (LOAD)</strong> / <strong className="text-[#22c55e]">2 inbound (RECEIVE)</strong> = 8 total (same set as 09-07 17:44 PDT — identical task IDs, doors, statuses and assignees). 75% outbound / 25% inbound. 5 doors carry at least one open task (21.7% task occupancy). No task opened, closed, or moved doors since the prior snapshot — only ages advanced (~56m).</li>
              ...
              <li><strong className="text-[#ef4444]">⚠ SEVERE ANOMALY — DOCK50 (unchanged):</strong> TASK-5090739 ~321d old (321d 5h at refresh; started Oct 21 2025, 13:21 PDT) RECEIVE task remains IN_PROGRESS with endTime set (2025-10-22) and its receipt RN-5002143 CLOSED. Assigned to daira gonzalez. Stuck/stale — investigate immediately. DOCK50 also hosts Arnulfo&apos;s LOAD TASK-5360206 (4d 2h, both loads LOADED) — two open tasks co-located.</li>
              <li><strong className="text-[#f59e0b]">⚠ Aged/Stale (unchanged):</strong> TASK-5338695 (DOCK54, Arnulfo) 31d 2h with endTime set (2026-08-10) and its load SHIPPED, yet task IN_PROGRESS.</li>
              <li><strong className="text-[#ef4444]">⚠ Co-location anomalies (unchanged):</strong> DOCK53 hosts two open tasks (TASK-5361270 LOAD IN_PROGRESS 3d 2h with 1 LOADED + 4 LOADING loads, and TASK-5360939 RECEIVE NEW — RUFINO MUNGUIA, receipt RN-191921 IMPORTED, not yet started); DOCK54 hosts two open LOAD tasks (TASK-5360934 IN_PROGRESS 3d 5h with all 8 loads LOADED + stale TASK-5338695).</li>
              <li><strong className="text-[#f59e0b]">ℹ LOADED-but-open watch:</strong> TASK-5360206 (DOCK50, 4d 2h), TASK-5359541 (DOCK51, 4d 8h) and TASK-5360934 (DOCK54, 3d 5h) are IN_PROGRESS with their loads already LOADED — not flagged under the endTime anomaly rule, but monitor for stalled dispatch/close-out. DOCK67&apos;s reserved task TASK-5359531 (LOAD NEW — JEROME ARANDA) still awaits its trailer: load LOAD-5037833 is WINDOW_CHECKIN_DONE with appointment 2026-09-08 09:00 PDT (Tue, APPT-6038673 CHECKED_IN — BEST YET EXPRESS) — scheduling, not an anomaly (re-verified ~18:40 PDT).</li>
              <li><strong className="text-[#22c55e]">★ Schedule % (today, 2026-09-07 — Monday, Labor Day, facility-wide):</strong> scheduled inbounds <strong>1 → 0%</strong> (RN-5009857, KING&apos;S HAWAIIAN, appointment 09-07 08:00, CHECKED_IN / in-yard 07:36:02 — OPEN, not yet received (receive task TASK-5361525 NEW — DOCK152, ERICK OLMEDO — created 07:29 PDT, outside Bay 4); verified live on 09-07 ~18:40 PDT); scheduled outbound loads <strong>0 → n/a</strong> (an OUTBOUND appointment entity 256422 — APPT-6037851, ORGAIN, LLC (ORG-655338), 13:00, CHECKED_IN — exists today but its load record LOAD-5037118 has appointmentTime 09-02T10:00 (already SHIPPED), so per the load-search convention it does not create a denominator). Facility-wide context: 9 receipts created today (RN-5010117/5010118 CANCELLED — TikTok Inc.; RN-191981/191983/191984/191985/191986/191987 IMPORTED — VAONIS; RN-191995 EXCEPTION — KARAKA, LLC), 0 receipts CLOSED/FORCE_CLOSED with receivedTime today, 0 loads created today (newest load facility-wide remains LOAD-5038142, NEW — created 09-06 00:37:59 PDT), 0 loads LOADED/SHIPPED with endTime today. Facility-wide scope (appointments carry no clean dockId for Bay 4 scoping).</li>
              <li><strong className="text-[#7c3aed]">★ All-time rollup rescanned:</strong> full CLOSED/FORCE_CLOSED load + receive scan at all 23 Bay-4 door IDs at ~18:40 PDT — 3,639 closed transactions (2,718 LOAD + 921 RECEIVE; 3,401 CLOSED + 238 FORCE_CLOSED) across 80 assignees, top assignee ARNULFO MUNGUIA 943. Identical to every rollup since 09-05 19:04 PDT (incl. 09-07 07:57/12:27/15:53/17:44 PDT) — no Bay-4 closures in the window (newest closed task endTime still 09-04T23:13:25 local = 2026-09-05T06:13:25Z; TASK-5361283, RECEIVE, DOCK58). (DANIEL BELTRAN 915, DANIELA GONZALEZ 366, GEORGE LC BROWN 151, RENATO ROSALES GARCIA 149, Caren Cubides 147, JULIO CESAR ALVARADO 111, MARTIN MUNGUIA 106, Fatima Del Rosario Ponce 88, David Ramirez Selva 76).</li>
              <li><strong className="text-[#7c3aed]">★ GuruNanda → Arnulfo rollup recomputed:</strong> 1,104 closed transactions facility-wide (1,102 LOAD + 2 RECEIVE); 901 at Bay 4 doors (900 LOAD + 1 RECEIVE) — identical to the 09-07 17:44 PDT snapshot (+0 in the window). Open: 5 LOAD IN_PROGRESS at DOCK50/51/53/54 (4 doors), ages at ~18:40 PDT (TASK-5360206 4d 2h, TASK-5359541 4d 8h, TASK-5361270 3d 2h, TASK-5360934 3d 5h, TASK-5338695 31d 2h ⚠STALE). Full pagination under assigneeUserId=89 + customerIds=[ORG-655875] + CLOSED/FORCE_CLOSED.</li>
              <li>All core metrics sourced from live WISE/WMS queries, Monday, September 7, 2026 ~18:40 PDT (all endpoints live — schedule metrics available). Assignee names via task assigneeUserName (exact API forms: ARNULFO MUNGUIA, RUFINO MUNGUIA, JEROME ARANDA, daira gonzalez); customers via task customer records (GURUNANDA, LLC per receipt/organization records). Durations: task startTime returned facility-local (item-time-zone America/Los_Angeles) and aged to the ~18:40 PDT snapshot instant — same convention as the Sep 4/5/6/7 snapshots.</li>
            </ul>
          </div>
        </section>
      </main>
      ...
      <footer className="border-t border-[#1e1e2a] bg-[#0a0a0f] mt-2">
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between text-xs text-[#71717a]">
          ...
          <span>Last refreshed: September 7, 2026 ~18:40 PDT</span>
        </div>
      </footer>
    </div>
  );
