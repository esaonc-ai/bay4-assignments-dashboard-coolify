    ...
    <div className="min-h-screen bg-[#0a0a0f]">
          ...
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <div className="bg-[#7c3aed10] border border-[#7c3aed33] rounded-lg px-4 py-3 mt-1">
              ...
              <span className="text-xs text-[#a1a1aa] block mt-0.5">
                ARNULFO MUNGUIA (userId=89) has {arnulfoOpenCount} open tasks at Bay 4 DOCK50–DOCK72 — {arnulfoOpenLoad} LOAD outbound + {arnulfoOpenReceive} RECEIVE inbound. {arnulfoOpenGuru} of those {arnulfoOpenCount} are GURUNANDA ({arnulfoOpenGuruLoad} LOAD + {arnulfoOpenGuruReceive} RECEIVE). All-time (GURUNANDA → Arnulfo): <strong className="text-[#f4f4f6]">{guruArnulfoAllTimeTotal}</strong> closed transactions at Bay-4 doors ({guruArnulfoAllTimeLoad} LOAD + {guruArnulfoAllTimeReceive} RECEIVE) — <strong className="text-[#f4f4f6]">recomputed in this refresh</strong> from a full row-level rescan (2026-09-24 ~09:07 PDT). Below is the current live snapshot (2026-09-24 ~09:07 PDT).
              </span>
            </div>
        ...
        <section>
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <ul className="text-xs text-[#71717a] space-y-1 list-disc list-inside">
              <li><strong className="text-[#f4f4f6]">7 Occupied / 2 Reserved / 14 Available</strong> — door utilization is task-derived at the 2026-09-24 ~09:07 PDT snapshot: 7 doors with an in-progress open task (DOCK50, DOCK51, DOCK52, DOCK54, DOCK55, DOCK57, DOCK62), 2 doors holding only not-started NEW tasks (DOCK53, DOCK59 — Reserved rather than Occupied), 14 doors with no open load/receive task. 9 doors carry an active assignment (39.1% of the bay). <strong className="text-[#f4f4f6]">Cross-check / correction candidate:</strong> the Location API&apos;s own <code>dockStatus</code> field reports 19 Bay-4 doors OCCUPIED / 4 AVAILABLE / 0 RESERVED (its <code>spaceStatus</code> reports 14 OCCUPIED / 9 EMPTY). It is driven by trailer check-in state rather than by open dock work and <strong className="text-[#f4f4f6]">diverges materially</strong> from task-derived status (7 vs 19) — notably DOCK55 and DOCK57 read AVAILABLE in the Location API while both carry IN_PROGRESS work. Task-derived status is retained as the authoritative basis for this dashboard, consistent with the door-duration metric.</li>
              <li>Open tasks: <strong className="text-[#7c3aed]">7 outbound (LOAD)</strong> / <strong className="text-[#22c55e]">7 inbound (RECEIVE)</strong> = 14 at ~09:07 PDT. 50.0% outbound / 50.0% inbound. The open population was read exhaustively — a full facility-wide sweep ({facilityOpenLoad} open load + {facilityOpenReceive} open receive tasks, statuses NEW/IN_PROGRESS/EXCEPTION) filtered on each row&apos;s <code>dockId</code> to the 23 Bay-4 doors.</li>
              <li><strong className="text-[#7c3aed]">Customer mix:</strong> 12 of the 14 open Bay-4 tasks are GURUNANDA, LLC (ORG-655875) — 85.7%; 2 are KARAKA, LLC (ORG-585450) — DOCK55 TASK-5373122, DOCK55 TASK-5368665. Task status: 10 IN_PROGRESS + 4 NEW.</li>
              <li><strong className="text-[#ef4444]">SEVERE ANOMALY — DOCK50 (aging 338d 2h 46m):</strong> TASK-5090739 RECEIVE remains IN_PROGRESS with endTime already set (2025-10-22) — stuck/stale and still aging. Assigned to daira gonzalez. Investigate immediately.</li>
              <li><strong className="text-[#f59e0b]">ANOMALY — DOCK54 (aging 47d 23h 37m):</strong> TASK-5338695 (LOAD, endTime 2026-08-10) remains IN_PROGRESS — stale — on a door that also carries a second open load task, TASK-5375520 (1d 4h 21m), for ARNULFO MUNGUIA.</li>
              <li><strong className="text-[#22c55e]">Single tight sweep:</strong> all figures on this page come from one self-consistent sweep at 2026-09-24T16:07:16Z (2026-09-24 ~09:07 PDT).</li>
              <li><strong className="text-[#22c55e]">Schedule % REPORTED this refresh:</strong> today is <strong className="text-[#f4f4f6]">Thursday 2026-09-24</strong> and the appointment windows for this facility-local day return live rows on both sides — <strong className="text-[#f4f4f6]">156 loads scheduled</strong> (10 loaded/shipped → <strong className="text-[#f4f4f6]">6.4%</strong>) and <strong className="text-[#f4f4f6]">31 receipts scheduled</strong> (3 received → <strong className="text-[#f4f4f6]">9.7%</strong>). Binding filters confirmed: <strong className="text-[#f4f4f6]">loads</strong> use <code>appointmentTimePeriod</code> — a BETWEEN that requires exactly two date-time elements, i.e. [&quot;2026-09-24T00:00:00&quot;,&quot;2026-09-24T23:59:59&quot;]; <strong className="text-[#f4f4f6]">receipts</strong> use <code>appointmentTimeFrom</code>/<code>appointmentTimeTo</code>. &quot;Loaded&quot; counts loads whose status shows loading complete — 7 SHIPPED + 3 LOADED = 10 (7 LOADING + 4 WINDOW_CHECKIN_DONE + 135 NEW still pending). &quot;Received&quot; counts receipts fully closed — 0 CLOSED + 3 FORCE_CLOSED = 3 (16 IMPORTED + 10 OPEN + 2 IN_PROGRESS still pending).</li>
              <li><strong className="text-[#22c55e]">All-time cumulative figures RECOMPUTED this refresh:</strong> the <strong className="text-[#f4f4f6]">{allTimeClosedTotal.toLocaleString()}</strong> closed Bay-4 transactions across <strong className="text-[#f4f4f6]">{allTimeDistinctAssignees}</strong> display names (top-10 list above) and the GURUNANDA → Arnulfo cumulative <strong className="text-[#f4f4f6]">{guruArnulfoAllTimeTotal}</strong> ({guruArnulfoAllTimeLoad} LOAD + {guruArnulfoAllTimeReceive} RECEIVE) come from a full row-level rescan performed in this refresh — 23 doors × 2 task types (2,841 LOAD + 1,019 RECEIVE, statuses CLOSED/FORCE_CLOSED). They are fresh, not carried forward.</li>
              <li><strong className="text-[#ef4444]">FILTER CORRECTION — status filter binds as the plural <code>statuses</code>; door filter binds only in the singular:</strong> on the task searches the status filter binds ONLY as the plural <code>statuses</code> (array); a <code>statusList</code> (array) is silently IGNORED. The door filter binds ONLY as the singular <code>dockId</code>; a plural <code>dockIds</code> (array) is silently IGNORED, and a comma-separated <code>dockId</code> string is rejected (HTTP 400). The open Bay-4 population was therefore resolved from a facility-wide sweep filtered on each row&apos;s <code>dockId</code>; the all-time block used a per-door read on the singular <code>dockId</code> (load-task as a string, receive-task as a number — both verified to bind).</li>
              <li><strong className="text-[#f59e0b]">TIMESTAMP BASIS — durations aged task startTime → snapshot instant:</strong> task timestamps are read on the same UTC frame as the snapshot instant (2026-09-24T16:07:16Z). Query window 2026-09-24T16:07:16Z → 2026-09-24T16:07:16Z.</li>
              ...
              <li><strong className="text-[#f59e0b]">Arnulfo identity caveat:</strong> uid <strong>89</strong> (amunguia) is the id on the live Bay-4 open and closed task rows for this display name — verified on all 8 open Bay-4 tasks assigned to &quot;ARNULFO MUNGUIA&quot; and on 100% of the closed Bay-4 rows carrying that name. It is the basis of the {guruArnulfoAllTimeTotal} all-time figure. Other employees may share the display name on different ids; merging them would inflate the figure, so the headline uses uid 89 only.</li>
              <li><strong className="text-[#7c3aed]">&quot;Guru live out / in assign to Arnulfo&quot; re-verified:</strong> still NO exact literal match — load-task and receive-task records expose no task-name field, so the interpretation holds: GURUNANDA, LLC (ORG-655875) dock transactions assigned to Arnulfo Munguia (assigneeUserId=89). Core live metrics sourced from live WISE/WMS queries — Thursday 2026-09-24 ~09:07 PDT (UTC snapshot 2026-09-24T16:07:16Z).</li>
            </ul>
          </div>
        </section>
      </main>
      ...
      <footer className="border-t border-[#1e1e2a] bg-[#0a0a0f] mt-2">
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between text-xs text-[#71717a]">
          ...
          <span>Last refreshed: {refreshDateLong} ~09:07 PDT</span>
        </div>
      </footer>
    </div>
  );
