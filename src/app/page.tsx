    ...
    <div className="min-h-screen bg-[#0a0a0f]">
        ...
        <section>
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <ul className="text-xs text-[#71717a] space-y-1 list-disc list-inside">
              <li><strong className="text-[#f4f4f6]">5 Occupied / 1 Reserved / 17 Available / 2 anomalies</strong> — door utilization is task-derived at the 2026-09-26 ~10:07 PDT snapshot: 5 doors with an in-progress open task (DOCK50, DOCK51, DOCK53, DOCK54, DOCK69), 1 door holding only a not-started NEW task (DOCK56 — Reserved rather than Occupied), 17 doors with no open load/receive task. 6 doors carry an active assignment (26.1% of the bay). <strong className="text-[#f4f4f6]">Cross-check / correction candidate:</strong> the Location API&apos;s own <code>dockStatus</code> field reports 14 Bay-4 doors OCCUPIED / 3 RESERVED / 6 AVAILABLE (its <code>spaceStatus</code> reports 14 OCCUPIED / 9 EMPTY). It is driven by trailer check-in state rather than by open dock work and <strong className="text-[#f4f4f6]">diverges materially</strong> from task-derived status (5 vs 14). Task-derived status is retained as the authoritative basis for this dashboard, consistent with the door-duration metric.</li>
              <li>Open tasks: <strong className="text-[#7c3aed]">4 outbound (LOAD)</strong> / <strong className="text-[#22c55e]">4 inbound (RECEIVE)</strong> = 8 at ~10:07 PDT. 50.0% outbound / 50.0% inbound. The open population was read exhaustively — a full facility-wide sweep ({facilityOpenLoad} open load + {facilityOpenReceive} open receive tasks, statuses NEW/IN_PROGRESS/EXCEPTION) filtered on each row&apos;s <code>dockId</code> to the 23 Bay-4 doors.</li>
              <li><strong className="text-[#7c3aed]">Customer mix:</strong> all 8 of the 8 open Bay-4 tasks are GURUNANDA, LLC (ORG-655875) — 100%; no KARAKA task is open at a Bay-4 door this snapshot. Task status: 7 IN_PROGRESS + 1 NEW.</li>
              <li><strong className="text-[#ef4444]">SEVERE ANOMALY — DOCK50 (aging 339d 20h 45m):</strong> TASK-5090739 RECEIVE remains IN_PROGRESS with endTime already set (2025-10-22) — stuck/stale and still aging. Assigned to daira gonzalez. DOCK50 also carries a second open task, TASK-5376774 (LOAD, IN_PROGRESS 1d 18h 34m) for ARNULFO MUNGUIA. Investigate immediately.</li>
              <li><strong className="text-[#f59e0b]">ANOMALY — DOCK54 (aging 49d 17h 37m):</strong> TASK-5338695 (LOAD, endTime 2026-08-10) remains IN_PROGRESS — stale — on a door that also carries TASK-5377435 (RECEIVE, IN_PROGRESS 1d 0h 0m) for RUFINO MUNGUIA.</li>
              <li><strong className="text-[#22c55e]">Single tight sweep:</strong> all current-state figures on this page come from one self-consistent sweep at 2026-09-26T17:06:58Z (2026-09-26 ~10:07 PDT).</li>
              <li><strong className="text-[#f59e0b]">Schedule % — zero-denominator day:</strong> today is <strong className="text-[#f4f4f6]">Saturday 2026-09-26</strong> and its appointment windows return <strong className="text-[#f4f4f6]">0 loads</strong> and <strong className="text-[#f4f4f6]">1 receipt</strong>. The outbound tile therefore reports <strong className="text-[#f4f4f6]">n/a</strong> (0 scheduled — zero denominator) and the inbound tile reports <strong className="text-[#f4f4f6]">0.0%</strong> (0 received of 1 scheduled; the single receipt is still IMPORTED). Nearest scheduled operating day is <strong className="text-[#f4f4f6]">2026-09-28</strong> (132 loads / 38 receipts). Binding filters confirmed: <strong className="text-[#f4f4f6]">loads</strong> use <code>appointmentTimePeriod</code> — a BETWEEN that requires exactly two date-time elements, i.e. [&quot;2026-09-26T00:00:00&quot;,&quot;2026-09-26T23:59:59&quot;]; <strong className="text-[#f4f4f6]">receipts</strong> use <code>appointmentTimeFrom</code>/<code>appointmentTimeTo</code>.</li>
              <li><strong className="text-[#22c55e]">All-time cumulative figures RECOMPUTED this refresh:</strong> the <strong className="text-[#f4f4f6]">{allTimeClosedTotal.toLocaleString()}</strong> closed Bay-4 transactions across <strong className="text-[#f4f4f6]">{allTimeDistinctAssignees}</strong> display names (top-10 list above) and the GURUNANDA → Arnulfo cumulative <strong className="text-[#f4f4f6]">{guruArnulfoAllTimeTotal}</strong> ({guruArnulfoAllTimeLoad} LOAD + {guruArnulfoAllTimeReceive} RECEIVE) come from a full row-level rescan performed in this refresh — 23 doors × 2 task types (2,866 LOAD + 1,026 RECEIVE, statuses CLOSED/FORCE_CLOSED). They are fresh, not carried forward.</li>
              ...
              <li><strong className="text-[#f59e0b]">TIMESTAMP BASIS — durations aged task startTime → snapshot instant:</strong> task timestamps are read on the same UTC frame as the snapshot instant (2026-09-26T17:06:58Z). Query window 2026-09-26T17:06:58Z → 2026-09-26T17:06:58Z.</li>
              ...
              <li><strong className="text-[#f59e0b]">Arnulfo identity caveat:</strong> uid <strong>89</strong> (amunguia) is the id on the live Bay-4 open and closed task rows for this display name — verified on both open Bay-4 tasks assigned to &quot;ARNULFO MUNGUIA&quot; and on 100% of the closed Bay-4 rows carrying that name. It is the basis of the {guruArnulfoAllTimeTotal} all-time figure. Other employees may share the display name on different ids; merging them would inflate the figure, so the headline uses uid 89 only.</li>
              <li><strong className="text-[#7c3aed]">&quot;Guru live out / in assign to Arnulfo&quot; re-verified:</strong> still NO exact literal match — load-task and receive-task records expose no task-name field, so the interpretation holds: GURUNANDA, LLC (ORG-655875) dock transactions assigned to Arnulfo Munguia (assigneeUserId=89). Core live metrics sourced from live WISE/WMS queries — Saturday 2026-09-26 ~10:07 PDT (UTC snapshot 2026-09-26T17:06:58Z).</li>
            </ul>
          </div>
        </section>
      </main>
      ...
      <footer className="border-t border-[#1e1e2a] bg-[#0a0a0f] mt-2">
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between text-xs text-[#71717a]">
          ...
          <span>Last refreshed: {refreshDateLong} ~10:07 PDT</span>
        </div>
      </footer>
    </div>
  );
