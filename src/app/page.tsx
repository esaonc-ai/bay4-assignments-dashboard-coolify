    ...
    <div className="min-h-screen bg-[#0a0a0f]">
        ...
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-xs text-[#71717a] tracking-wide">
              DOCK50–DOCK72 &nbsp;|&nbsp; September 6, 2026 &nbsp;|&nbsp; Last refreshed: Sep 06 11:30 PDT
            </p>
          </div>
          {/* Facility badge */}
          ...
          <div className="mt-3 bg-[#141419] border border-[#1e1e2a] rounded-xl overflow-hidden">
            <div className="px-5 py-2.5 bg-[#0a0a0f] border-b border-[#1e1e2a]">
              <span className="text-xs font-semibold text-[#71717a] uppercase tracking-wider">
                All-Time Assignments (DOCK50–DOCK72) — recomputed 09/06 11:30 PDT
              </span>
            </div>
          ...
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <div className="bg-[#7c3aed10] border border-[#7c3aed33] rounded-lg px-4 py-3 mt-1">
              ...
              <span className="text-xs text-[#a1a1aa] block mt-0.5">
                ...
                (901 at Bay 4 doors) — fresh rollup, 2026-09-06 11:30 PDT. Below is the current snapshot.
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-[#a1a1aa] mt-1">
                  <strong>DOCK50:</strong> TASK-5360206 (LOAD, IN_PROGRESS, 2d 19h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK51:</strong> TASK-5359541 (LOAD, IN_PROGRESS, 3d 1h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK53:</strong> TASK-5361270 (LOAD, IN_PROGRESS, 1d 19h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
                  <strong>DOCK54:</strong> TASK-5360934 (LOAD, IN_PROGRESS, 1d 22h)
                </span>
                <span className="text-xs text-[#a1a1aa]">
        ...
        <section>
          <div className="bg-[#141419] border border-[#1e1e2a] rounded-xl p-5 flex flex-col gap-2">
            ...
            <ul className="text-xs text-[#71717a] space-y-1 list-disc list-inside">
              <li><strong className="text-[#f4f4f6]">4 Occupied / 1 Reserved / 18 Available</strong> — door utilization sourced from open load/receive tasks: 4 doors with an active IN_PROGRESS task (DOCK50,51,53,54), 1 door with a NEW not-yet-started task (DOCK67), 18 doors with no open load/receive task. <strong className="text-[#a1a1aa]">Unchanged since the 09-05 19:04 PDT snapshot</strong> — no door opened, freed, or moved tasks in the window (~16.5h).</li>
              <li>Open tasks: <strong className="text-[#7c3aed]">6 outbound (LOAD)</strong> / <strong className="text-[#22c55e]">2 inbound (RECEIVE)</strong> = 8 total (same set as 09-05 19:04). 75% outbound / 25% inbound. 5 doors carry at least one open task (21.7% task occupancy). No task opened or closed since the prior snapshot — only ages advanced (~16h 26m).</li>
              ...
              <li><strong className="text-[#ef4444]">⚠ Co-location anomalies (unchanged):</strong> DOCK53 hosts two open tasks (TASK-5361270 LOAD IN_PROGRESS 1d 19h with 1 LOADED + 4 LOADING loads, and TASK-5360939 RECEIVE NEW — RUFINO MUNGUIA, receipt RN-191921 IMPORTED, not yet started); DOCK54 hosts two open LOAD tasks (TASK-5360934 IN_PROGRESS 1d 22h with all 8 loads LOADED + stale TASK-5338695).</li>
              <li><strong className="text-[#f59e0b]">ℹ LOADED-but-open watch:</strong> TASK-5360206 (DOCK50, 2d 19h), TASK-5359541 (DOCK51, 3d 1h) and TASK-5360934 (DOCK54, 1d 22h) are IN_PROGRESS with their loads already LOADED — not flagged under the endTime anomaly rule, but monitor for stalled dispatch/close-out.</li>
              <li><strong className="text-[#22c55e]">★ Schedule % (today, 2026-09-06 — Sunday, facility-wide):</strong> scheduled inbounds <strong>0 → n/a</strong> (no receipt carries an appointment time of 09-06; appointment search returns 0 appointment entities today — verified working via the 09-05 window, which returned 3: RN-5010097 GURUNANDA, RN-5009845 CMPC USA INC, RN-5009641 AMIEE LYNN, LNC.); scheduled outbounds <strong>0 → n/a</strong> (no Sunday load appointments exist). Facility-wide context: 0 receipts created today, 0 receipts CLOSED/FORCE_CLOSED with receivedTime today, 1 load created today (LOAD-5038142, WOODY FLAW CREST INC, NEW — createdTime 2026-09-06T07:37:59Z = 00:37:59 PDT), 0 loads LOADED/SHIPPED with endTime today. Facility-wide scope (appointments carry no clean dockId for Bay 4 scoping).</li>
              <li><strong className="text-[#7c3aed]">★ All-time rollup rescanned:</strong> full CLOSED/FORCE_CLOSED load + receive scan at all 23 Bay-4 door IDs at 11:30 PDT — 3,639 closed transactions (2,718 LOAD + 921 RECEIVE; 3,401 CLOSED + 238 FORCE_CLOSED) across 80 assignees, top assignee ARNULFO MUNGUIA 943. Identical to the 09-05 19:04 PDT rollup — no Bay-4 closures since (newest closed task endTime 2026-09-05T06:13:25Z = 09-04 23:13 PDT). (DANIEL BELTRAN 915, DANIELA GONZALEZ 366, GEORGE LC BROWN 151, RENATO ROSALES GARCIA 149, Caren Cubides 147, JULIO CESAR ALVARADO 111, MARTIN MUNGUIA 106, Fatima Del Rosario Ponce 88, David Ramirez Selva 76).</li>
              <li><strong className="text-[#7c3aed]">★ GuruNanda → Arnulfo rollup recomputed:</strong> 1,104 closed transactions facility-wide (1,102 LOAD + 2 RECEIVE — re-verified with the customerIds array filter, since the load-task endpoint silently ignores the singular customerId field); 901 at Bay 4 doors (900 LOAD + 1 RECEIVE) — identical to 09-05 19:04 PDT (+0 in the window). Open: 5 LOAD IN_PROGRESS at DOCK50/51/53/54 (4 doors), ages advanced ~16.5h (TASK-5360206 2d 19h, TASK-5359541 3d 1h, TASK-5361270 1d 19h, TASK-5360934 1d 22h, TASK-5338695 29d 19h ⚠STALE). Full pagination under assigneeUserId=89 + customerIds=[ORG-655875] + CLOSED/FORCE_CLOSED.</li>
              <li>All core metrics sourced from live WISE/WMS queries, Sunday, September 6, 2026 ~11:30 PDT. Assignee names via task assigneeUserName (exact API forms: ARNULFO MUNGUIA, RUFINO MUNGUIA, JEROME ARANDA, daira gonzalez); customers via task customer records (GURUNANDA, LLC per receipt/organization records). Durations: API startTime treated as UTC, converted to America/Los_Angeles (UTC-7) — same convention as the Sep 4/5 snapshots.</li>
            </ul>
          </div>
        </section>
      </main>
      ...
      <footer className="border-t border-[#1e1e2a] bg-[#0a0a0f] mt-2">
        <div className="max-w-[1440px] mx-auto px-6 py-4 flex items-center justify-between text-xs text-[#71717a]">
          ...
          <span>Last refreshed: September 6, 2026 ~11:30 PDT</span>
        </div>
      </footer>
    </div>
  );
